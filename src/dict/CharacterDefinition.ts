...
import isHighSurrogate from "../util/is-high-surrogate";
...
export default class {
   ...
   * @returns {}
   */
  loadg(cat_map_buffer, compat_cat_map_buffer, invoke_def_buffer) {
  ...
  parseCharCategoryg(class_id, parsed_category_def) {
   ...
   */
  static initCategoryMappingsg(category_mapping) {
      ...
      for (let i = 0; i < category_mapping.length; i++) {
        ...
        for (code_point = mapping.start; code_point <= end; code_point++) {
          ...
          for (let j = 0; j < mapping.compatible.length; j++) {
            ...
            bitset |= class_id_bit; // Set a bit of class ID 例えば、class_idが3のとき、3ビット目に1を立てる
   ...
   */
  static lookupCompatibleCategoryg(ch: string) {
    const classes = [];
